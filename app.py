...
# ==============================================================================
def check_agreement():
    """利用規約への同意を確認し、未同意なら同意画面を表示してFalseを返す"""
    ...
    # --- ここから同意画面のデザイン ---
    st.markdown("## 🔒 利用に際しての確認")

    # 注意文言（st.warning内でMarkdownを使って強調）
    st.warning("""
    # ⚠️ 【重要：利用について】 ⚠️
    
    ### 🛑 以下のルールを【厳守】できる方のみご利用ください
    
    ユロっとおにーは戦略詳細が広く知られると、**スプレッド拡大など優位性が損なわれるリスク**があります。
    
    ---
    
    ### 🚫 SNSにユロっとおにーの戦略詳細に関する投稿（特にv7）をしない
    
    ### 🤝 このツールは個人利用のみとし部外者への共有はしない
    """)

    # 同意ボタン
    if st.button("上記内容に同意して利用を開始する", type="primary"):
...
# ==============================================================================
def main():
    # 同意していない(False)場合、ここで return して処理を強制終了する
    if not check_agreement():
        return  # ← これがないと、下の画面も一緒に表示されてしまいます
